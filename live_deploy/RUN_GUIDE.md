...
## AWS deployment: SSM Parameter Store

AWS is now the chosen hosting target, so this is that "real secrets
manager" step the previous section pointed at, rather than env vars in
yet another file on disk. **AWS Systems Manager Parameter Store**
(SecureString) — free for standard-tier parameters, no extra service to
run, encrypted at rest via KMS, and access is controlled by IAM rather
than a file's Unix permissions. `config.json` isn't needed on the
instance at all with this in place — `load_config()` already treats it
as fully optional once every required key arrives as an env var (see
"Credential hardening" above).

**One-time setup** (from your own machine, with the AWS CLI configured):

```bash
aws ssm put-parameter --type SecureString --name /live-deploy/KITE_API_KEY       --value "..."
aws ssm put-parameter --type SecureString --name /live-deploy/KITE_API_SECRET    --value "..."
aws ssm put-parameter --type SecureString --name /live-deploy/DATABASE_URL       --value "..."
aws ssm put-parameter --type SecureString --name /live-deploy/APP_AUTH_SECRET    --value "..."
```

**IAM policy** — attach to the EC2 instance's role (or the ECS task
role), scoped to just this path, nothing broader:
...
```json
{
  ...
  "Statement": [{
    ...
    "Resource": "arn:aws:ssm:*:*:parameter/live-deploy/*"
  }]
}
```

(If the parameters use the default AWS-managed KMS key `alias/aws/ssm`,
that's it — the SSM service handles decryption using the caller's own
`ssm:GetParameters` permission. A **customer-managed** KMS key would
additionally need `kms:Decrypt` on that specific key in the same
policy.)

**`scripts/fetch_ssm_secrets.sh`** — fetches all four in one API call,
exports them as env vars, and `exec`s whatever command you give it, so
the actual Python process never touches a secrets file at all:

```bash
./scripts/fetch_ssm_secrets.sh python3 run.py
```

Wiring it into each option above:

- **Option 2 (supervisord)**: change `supervisord.conf`'s `command=`
  line to
  `command=%(here)s/scripts/fetch_ssm_secrets.sh python3 %(here)s/run.py`
- **Option 2 (systemd)**: same idea —
  `ExecStart=/path/to/live_deploy/scripts/fetch_ssm_secrets.sh python3 /path/to/live_deploy/run.py`
  — no `EnvironmentFile=` needed at all with this in place.
- **Option 3 (Docker)**: replace the image's `CMD` with an entrypoint
  that runs the script — e.g. add
  `COPY scripts/ ./scripts/` to the `Dockerfile` and set
  `ENTRYPOINT ["./scripts/fetch_ssm_secrets.sh"]` /
  `CMD ["python", "run.py"]`. The container then needs no `-v
  config.json:...` mount and no `-e KITE_API_KEY=...` flags — just an
  IAM role available to the container (an EC2 instance profile if
  running plain Docker on EC2, or a task role if this ever moves to
  ECS/Fargate) and the AWS CLI present in the image (`RUN pip install
  awscli` or a base image that already has it).

`SSM_PARAM_PREFIX` (env var, defaults to `/live-deploy`) lets one AWS
account host more than one environment (e.g. `/live-deploy-staging`)
under separate parameter paths without editing the script.

**Verified end-to-end**: the REAL app — full FastAPI lifespan,
migrations, dispatcher, deployment manager, not just `load_config()` in
isolation — booted successfully through `fetch_ssm_secrets.sh` against
a mocked SSM (a stand-in `aws` CLI on `PATH`), with `config.json`
genuinely absent from disk. The SSM-sourced `app_auth_secret` was
confirmed LIVE the same way the env-var path was above — the correct
value authorizes, a wrong one still 401s — and `GET /health` returned
healthy against the SSM-sourced `DATABASE_URL`. Also verified the
failure paths: a wrong `SSM_PARAM_PREFIX` (parameters not found under
that path) fails loudly with a clear "missing from SSM" message listing
exactly which keys weren't found, rather than silently launching with
empty credentials; running with no command argument prints a usage
