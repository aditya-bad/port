...
async def get_pnl_digest(
    ...
    max_profit/max_loss (Step 100, corrected in Step 101, corrected
    again in Step 102): only list_pnl_digest_for_deployment (the
    non-year-scoped path, i.e. the Stats "Recent Periods" trend table)
    computes these -- see that function's own docstring for the
    mode-dependent mechanism. For a "positional" deployment
    specifically, `period` is silently ignored and each row is one
    EPISODE -- every position/leg/adjustment/roll that overlapped in
    time, combined into one (not a calendar bucket, and not a single
    `positions` table row either) -- the Calendar heatmap never hits
    that branch (it always passes `year`), so this doesn't affect it
    either way.
    """
    pool = request.app.state.db_pool
    dep = await queries.get_deployment(pool, deployment_id)
