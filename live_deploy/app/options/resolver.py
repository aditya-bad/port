...
class OptionsResolver:
    ...
    async def get_balanced_straddle_strike(
        self, underlying: str, expiry_selector: Union[str, int, date],
        exchange: Optional[str] = None, strike_window: int = 15,
    ) -> float:
        """
        The strike (within `strike_window` strike-steps of the ordinary
        spot-rounded ATM strike) whose CE and PE premiums are closest to
        EACH OTHER — the actual forward-implied "fair" straddle strike,
        not simply whichever strike happens to be nearest spot.

        WHY spot-ATM isn't the same as "balanced": options price off
        the underlying's FORWARD/FUTURE, not spot. By put-call parity,
        CE(K) - PE(K) ~= Future - K for any strike K — so the spot-
        rounded strike (which implicitly assumes Future == Spot) carries
        a real premium imbalance whenever there's genuine cost-of-carry
        between spot and forward (bigger the further out from expiry,
        shrinking toward ~0 right at expiry as the future converges to
        spot). Picking the strike where CE and PE are closest to equal
        is picking the strike closest to the ACTUAL forward price —
        without needing a real futures contract to reference at all,
        which matters here since weekly index options have no matching
        weekly future listed (only the monthly one exists).

        Falls back to the plain spot-ATM strike if no candidate in the
        window has live quotes on BOTH sides (a very illiquid/thin
        chain) — never raises just because the "better" answer isn't
        available.
        """
        ...
        step = await self.get_strike_step(underlying, expiry, exchange)
        chain = await self.list_option_chain(underlying, expiry, exchange)
        window = [s for s in chain if abs(s - atm) <= strike_window * step + step / 2]
        candidates = window or list(chain.keys())

        keys = []
        for s in candidates:
            pair = chain[s]
            if pair["CE"] is not None:
                keys.append(pair["CE"].key)
            if pair["PE"] is not None:
                keys.append(pair["PE"].key)
        kite = self._kite()
        ...
        for s in candidates:
            pair = chain[s]
            if pair["CE"] is None or pair["PE"] is None:
                continue
            ce_entry = ltp_resp.get(pair["CE"].key)
            pe_entry = ltp_resp.get(pair["PE"].key)
            if not ce_entry or not pe_entry:
                continue
            diff = abs(ce_entry["last_price"] - pe_entry["last_price"])
            # Strictly smaller only (same ascending-strike, lowest-wins
            # tie-break convention as get_leg_by_premium) — `chain` is
            # already strike-sorted, so the first candidate to reach a
            # given diff keeps it.
            if best_diff is None or diff < best_diff:
                best_strike, best_diff = s, diff

        return best_strike if best_strike is not None else atm
